...
(function () {
  ...
  const HINTS = {
    bedroom: '点下面的家具放进房间，用手指拖一拖，不想要的拖到小筐里',
    living: '这是客厅！摆上沙发和电视吧',
    bathroom: '这是卫生间！点点浴缸会有泡泡哦'
  };
  const PRAISE = ['好漂亮呀！', '房间真好看！', '你真棒！', '布置得真好！'];

  window.makeRoomModule = function (roomId) {
    let el = null;
    let cleanMode = false;
    let palettePop = null;   // 当前打开的调色板
    let drag = null;

    const roomState = () => Store.state.rooms[roomId];
    ...
    function renderItems() {
      ...
      roomState().items.forEach((it, i) => area.appendChild(makeItemEl(it, i)));
    }
    ...
    function addItem(id) {
      ...
      const it = {
        ...
        y: def.flat || !['frame', 'clock', 'bmirror', 'towelrack'].includes(id) ? 0.55 + Math.random() * 0.2 : 0.22 + Math.random() * 0.14,
      };
      roomState().items.push(it);
      Store.save();
      const node = makeItemEl(it, roomState().items.length - 1);
    ...
    function playInteract(node, act) {
      ...
      if (act === 'bath') {
        ...
        FX.sparkles(el, cx, cy, 9);
        Sound.sparkle();
        Sound.praiseRandom(['动画片开始啦！', '电视亮啦！']);
      } else if (act === 'fish') {
        FX.bubbles(el, cx, cy, 6);
        Sound.blub();
    ...
    function onPointerDown(e) {
      ...
      const it = roomState().items[+node.dataset.i];
      drag = {
        node, area, rect, it,
    ...
    function onPointerUp(e) {
      ...
      if (drag.moved) {
        ...
        playInteract(drag.node, def.act); // 轻点不拖 = 互动
      }
      drag = null;
      basket = null;
    }
    ...
    function togglePalette(kind) {
      ...
      const cur = kind === 'wall' ? roomState().wall : roomState().floor;
      ...
      defs.forEach((def, i) => {
        ...
        s.addEventListener('click', () => {
          if (kind === 'wall') roomState().wall = i;
          else roomState().floor = i;
          Store.save();
          applyDecor();
          Sound.chime();
    ...
    return {
      ...
      onEnter() {
        renderAll();
        window.hintOnce('room-' + roomId, HINTS[roomId] || HINTS.room);
      },
      onLeave() {
        cleanMode = false;
  ...
  /* 卧室沿用原屏幕 id 'room'，客厅/卫生间新屏幕；roomId 即存档键 */
  window.Room = window.makeRoomModule('bedroom');
  window.LivingRoom = window.makeRoomModule('living');
  window.Bathroom = window.makeRoomModule('bathroom');
})();
